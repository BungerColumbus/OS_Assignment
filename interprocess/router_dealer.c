...
void cleanup(void)
{ 
    // Generic cleanup function for exit
...
{ 
    // Cleanup function in case of input during program run
    cleanup();
    _exit(0);
}
...
int main(int argc, char *argv[])
{
    ...
    // Naming the file paths to the queues according to the
    // assignment specificiation. Our group number is included
    sprintf(client2dealer_name, "/mq_request_%s_%d", "Group44", getpid());  
    sprintf(dealer2worker1_name, "/mq_worker1_%s_%d", "Group44", getpid());
    ...
    else
    {
        ...
        for (int i = 0; i < N_SERV2; i++)
        {
            ...
            { 
                ...
                if (processID < 0)
                {
                    perror("worker2 fork() failed"); // Check if the fork failed
                    exit(1);
                }
        ...
        fds[0].fd = mq_fd_req;
        fds[0].events = (!has_pending_S1 && !has_pending_S2) ? POLLIN : 0; // Only read if sending to worker queue will work
        fds[1].fd = mq_fd_rep;
        fds[1].events = POLLIN; // Always listen for responses
        fds[2].fd = mq_fd_S1;
        fds[2].events = has_pending_S1 ? POLLOUT : 0; // Only check space if a message needs to be resent
        fds[3].fd = mq_fd_S2;
        ...
        while (client_alive || reqCounter > 0 || has_pending_S1 || has_pending_S2 || messages_in_queue > 0)
        { 
            // Handle request and response messages while client is active
            if (client_alive)
            {
            ...
            // Exit BEFORE polling if everything is done
                    ...
                    if (bytes_read > 0)
                    {
                        ...
                        if (req.service_id == 1)
                        {
                            ...
                            else if (errno == EAGAIN)
                            {
                                ...
                                pending_S1 = ser;
                                sent_or_buffered = true;
                                // If worker queue is full, stop draining requests
            ...
            // Check for exit every iteration, regardless of poll outcome
            if (!client_alive && reqCounter <= 0 && !has_pending_S1 && !has_pending_S2)
            {
            ...
            // Debug state (this will now actually print during the hang)
            if (!client_alive)
            {
        ...
        //  Send signal to terminate to all workers
        ...
        // Close the message queues. The unlinking happens at exit
